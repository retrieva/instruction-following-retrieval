...
今回は、指示追従検索の性能改善に向けて2種類の重み付き対照学習とマージンロスの手法について検証します。

## 検証した手法
### 重み付き対照学習（手法1）
正例文書に対して、入力テキストの関連度を考慮した重み付き対照学習です。
重み付き対照学習(手法1)では、正例文書に対して「クエリ・指示文・クエリ+指示文」の3つを正解ラベルとします。
[図1](#fig-1)のように正例文書と入力テキストの関連度を重みとし、損失に掛け合わせます。
...
通常の対照学習では、「クエリ+指示文と文書」のみを対象として訓練していましたが、クエリや指示文の単体だけでも正例文書に関連しています。
...
<p align="center" id="fig-2">
  <img src="https://github.com/retrieva/2025_internship/blob/main/images/weighted_v2.png" width="70%" alt="重み付き対照学習（手法2）">
  <figcaption><p align="center">図2：重み付き対照学習（手法2）の概要図</p></figcaption>
</p>

### マージンロス
教師ありSimCSEを用いてクエリ+指示文と負例文書の類似度を計測し、その類似度が高い場合、マージンの値が大きくなるマージンロスです。
以下の式に従って計算されています。
...
[株式会社レトリバの技術ブログ](https://zenn.dev/retrieva_tech/articles/fa07c72b132921)にも紹介してありますので、ご確認ください。
