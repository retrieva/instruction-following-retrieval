...
def main(cfg: DictConfig):
    ...
    try:
        cfg_local_rank = cfg.get("local_rank", -1)
    except Exception:
        try:
            cfg_local_rank = cfg.distributed.get("local_rank", -1)
        except Exception:
            cfg_local_rank = -1

    local_rank = local_rank_env if local_rank_env != -1 else cfg_local_rank
    ...
    training_args = TrainingArguments(
        ...
        deepspeed=cfg.training.deepspeed_cfg
    )
